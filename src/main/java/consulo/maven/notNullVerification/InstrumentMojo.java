...
public class InstrumentMojo extends AbstractMojo
{
	...
	public void execute() throws MojoExecutionException, MojoFailureException
	{
		try
		{
			...
			File sourceDirectoryFile = new File(outputDirectory);
			...
			List<File> files = FileUtils.getFiles(sourceDirectoryFile, "**/*.class", null);
			if(files.isEmpty())
			{
				return;
			}
			...
			for(File classFile : files)
			{
				...
				boolean modified = NotNullVerifyingInstrumenter.processClassFile(reader, writer);
				if(modified)
				{
					changed = true;
					...
					try
					{
						...
						IOUtil.copy(writer.toByteArray(), outputStream);

						cacheLogic.putCacheEntry(classFile);
					}
					finally
					{
